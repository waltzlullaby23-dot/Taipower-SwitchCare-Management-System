-- SwitchCare Enterprise v10.2 FINAL
-- Corrects the permission denied for table "switches" encountered when
-- creating a device from the browser.
--
-- Design:
--   Browser can SELECT via RLS.
--   Browser cannot directly INSERT/UPDATE/DELETE business tables.
--   All business mutations go through SECURITY DEFINER functions.
-- This keeps the API surface controlled while preserving auditability.
--
-- Existing data is preserved. No DROP TABLE.
...
-- ============================================================
-- 2. Remove old triggers and business functions safely
-- ============================================================

do $$
declare r record;
begin
  -- Remove all old triggers on our four tables whose names or trigger
  -- functions belong to previous SwitchCare versions.
  for r in
    ...
    where not t.tgisinternal
      ...
      and c.relname in ('switches','charge_records','usage_records')
      and (
        t.tgname ilike '%switch%'
        or t.tgname ilike '%charge%'
        or t.tgname ilike '%usage%'
        or p.proname in ('touch_updated_at','audit_switch')
      )
  loop
    execute format('drop trigger if exists %I on %I.%I;',r.trigger_name,r.schema_name,r.table_name);
  end loop;

  -- Remove old business functions by exact identity signature.
  for r in
    select p.oid,
           format('drop function if exists %I.%I(%s);',
                  n.nspname,p.proname,pg_get_function_identity_arguments(p.oid)) as ddl
    from pg_proc p
    join pg_namespace n on n.oid=p.pronamespace
    where n.nspname='public'
      and p.proname in (
        'create_switch','update_switch_master','issue_switch',
        'return_switch','send_switch_for_charge','complete_switch_charge',
        'log_event','audit_switch'
      )
  loop
    execute r.ddl;
  end loop;
...
-- Do not drop helper functions. Replace them in place.
...
-- ============================================================
-- 3. Audit trigger (SECURITY DEFINER)
-- ============================================================
...
begin
  if tg_op='INSERT' then
    insert into public.audit_log(
      switch_id,event_type,material_no,taipower_no,
      actor_id,actor_email,new_data,note
    )
    values(
...
-- ============================================================
-- 4. Business functions (SECURITY DEFINER)
-- ============================================================
...
begin
  ...
  if exists(select 1 from public.switches where taipower_no=trim(p_taipower_no)) then
    raise exception '台電編號已存在：%',trim(p_taipower_no);
  end if;

  v_next:=(p_entry_date+interval '6 months')::date;

  insert into public.switches(
    material_no,taipower_no,type,rating_type,warehouse,location,
    entry_date,state,cycle_start_date,next_charge_date,remark
  )
  values(
    p_material_no,trim(p_taipower_no),trim(p_type),p_rating_type,
    nullif(trim(p_warehouse),''),nullif(trim(p_location),''),
  ...
  values(
    v_switch.id,1,p_entry_date,v_next,'待充電','首次入帳，建立6個月週期'
  );
...
begin
  ...
  if p_rating_type not in ('新品','舊品') then raise exception '評價類型只能是新品或舊品'; end if;
  if coalesce(trim(p_type),'')='' then raise exception '型式不可空白'; end if;
  if exists(select 1 from public.switches where taipower_no=trim(p_taipower_no) and id<>p_switch_id) then
    raise exception '台電編號已存在：%',trim(p_taipower_no);
  end if;

  update public.switches
  set material_no=p_material_no,
      taipower_no=trim(p_taipower_no),
      type=trim(p_type),
      rating_type=p_rating_type,
      warehouse=nullif(trim(p_warehouse),''),
...
begin
  ...
  if found then
    ...
    set status='因領用中止',note=coalesce(p_note,'設備領用，停止本次在庫充電週期')
  ...
  perform public.log_event(
    v_new,'領用',p_document_no,'領用後停止6個月充電計時',to_jsonb(v_old),to_jsonb(v_new)
  );
end;
$$;
...
begin
  ...
  order by issue_date desc limit 1 for update;

  if found then
    update public.usage_records
    set return_date=p_event_date,status='已退庫',note=coalesce(p_note,note)
    where id=v_usage.id;
  else
    insert into public.usage_records(
      switch_id,issue_date,issue_no,return_date,status,note
    )
    values(
      p_switch_id,coalesce(v_old.issue_date,p_event_date),v_old.issue_no,
      p_event_date,'已退庫','找不到對應領用紀錄，系統補建歷史紀錄'
    );
  end if;

  select coalesce(max(cycle_no),0)+1 into v_cycle
  from public.charge_records where switch_id=p_switch_id;
  ...
  values(
    p_switch_id,v_cycle,p_event_date,v_next,'待充電','退庫視同重新入庫，重新起算6個月'
  );

  perform public.log_event(
    v_new,'退庫',p_document_no,'退庫後視同重新入庫並重新起算6個月',
...
begin
  ...
  update public.switches
  set state='送檢充電',send_date=p_event_date,transfer_no=trim(p_transfer_no),remark=p_note
  where id=p_switch_id
  returning * into v_new;
  ...
  if found then
    update public.charge_records
    set status='送檢中',send_date=p_event_date,transfer_no=trim(p_transfer_no),note=p_note
    where id=v_charge.id;
  else
    insert into public.charge_records(
  ...
  perform public.log_event(
    v_new,'送檢充電',trim(p_transfer_no),coalesce(p_note,'送檢檢修課充電'),
    to_jsonb(v_old),to_jsonb(v_new)
  );
end;
$$;
...
begin
  ...
  if found then
    update public.charge_records
    set status='已完成',completed_date=p_event_date,
    ...
    values(
      ...
      p_event_date,v_old.send_date,coalesce(p_transfer_no,v_old.transfer_no),
      p_event_date,'已完成',p_note
    );
  end if;
  ...
  values(
    p_switch_id,v_cycle,p_event_date,v_next,'待充電','完成充電後建立下一個6個月週期'
  );

  perform public.log_event(
    v_new,'充電完成',coalesce(p_transfer_no,v_old.transfer_no),
...
-- Explicitly prevent browser direct writes; RPCs are the controlled write path.
...
-- Do not expose the internal audit helper directly.


-- ============================================================
-- Internal helper execute permissions
-- Use pg_catalog to avoid hard-coded composite type signatures.
-- ============================================================
...
begin
  for r in
    ...
    join pg_namespace n on n.oid=p.pronamespace
    where n.nspname='public'
      and p.proname in ('log_event','audit_switch')
  loop
    execute format(
...
select
  s.id,1,s.cycle_start_date,s.next_charge_date,'待充電',
  '舊版設備資料回填之第一個充電週期'
from public.switches s
where s.cycle_start_date is not null
  and s.next_charge_date is not null
  and not exists(
    select 1 from public.charge_records c where c.switch_id=s.id
  );

-- ============================================================
-- 7. Final grant sanity checks
-- ============================================================

-- Note: no DROP TABLE occurs in this script.
