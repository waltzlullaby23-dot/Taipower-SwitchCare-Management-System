...
returns text language sql stable as $$ select coalesce((auth.jwt()->>'email'),'system') $$;
...
create trigger trg_switches_audit after insert or update on public.switches for each row execute function public.audit_switch();
